...
import com.github.conanchen.gedit.store.profile.grpc.BanRequest;
import com.github.conanchen.gedit.store.profile.grpc.CreateRequest;
import com.github.conanchen.gedit.store.profile.grpc.GetRequest;
import com.github.conanchen.gedit.store.profile.grpc.UpdateRequest;
...
import static com.github.conanchen.gedit.store.profile.grpc.StoreProfileApiGrpc.*;

@Slf4j
@Component
public class ProfileInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(ServerCall<ReqT, RespT> call, Metadata headers, ServerCallHandler<ReqT, RespT> next) {
        final MethodDescriptor methodDescriptor = call.getMethodDescriptor();
        ...
        return new ForwardingServerCallListener.SimpleForwardingServerCallListener<ReqT>(original) {
            ...
            public void onMessage(ReqT message) {
                try {
                    if (methodDescriptor.equals(getCreateMethod())) {
                        checkCreate((CreateRequest) message);
                    } else if (methodDescriptor.equals(getUpdateMethod())) {
                        checkUpdate((UpdateRequest) message);
                    } else if (methodDescriptor.equals(getGetMethod())) {
                        checkGet((GetRequest) message);
                    } else if (methodDescriptor.equals(getBanMethod())) {
                        checkBan((BanRequest) message);
                    }
                    super.onMessage(message);
                }catch (UncheckedValidationException e){
                    log.info("invalid args；[{}]",e.getMessage());
    ...
    private void checkUpdate(UpdateRequest req){
        //uuid
        Hope.that(req.getUuid()).isPresent();
        //logo
    ...
    private void checkGet(GetRequest req){
        Hope.that(req.getUuid()).isPresent();
    }
    ...
    private void checkLocation(Location location){
        if (location.getLat() != 0.0D && location.getLon() != 0.0D){
            // ignore
        }else if (location.getLat() == 0.0D && location.getLon() == 0.0D){
            // ignore
        }else{
            throw new UncheckedValidationException("location may lat or lon null");
        }
    }
